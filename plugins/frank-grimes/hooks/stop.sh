...
#
# This hook intercepts Claude's exit attempts during an active grind loop.
...
# (Previously used SESSION_ID-based path which broke if PWD changed)
STATE_FILE="${CACHE_DIR}/sessions/grimes-state.json"
...
if ! command -v jq &> /dev/null; then
    log "ERROR: jq not found. Auto-loop feature requires jq. Install with: brew install jq (macOS) or apt-get install jq (Linux)"
...
if [[ ! -f "$STATE_FILE" ]]; then
    log "No active grind state found"
    exit 0
fi
...
# Exit with code 2 to signal Claude should continue
exit 2
